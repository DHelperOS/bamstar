...
class PhoneNumberTextInputFormatter extends TextInputFormatter {
  ...
  formatted = digits.replaceFirstMapped(RegExp(r'^(\d{3})(\d+)'), (m) => '${m.group(1)}-${m.group(2)}');
    } else {
      // 3-4-... grouping (max 11 digits -> 3-4-4)
      formatted = digits.replaceFirstMapped(RegExp(r'^(\d{3})(\d{4})(\d*)'), (m) {
        final tail = m.group(3) ?? '';
        return tail.isEmpty ? '${m.group(1)}-${m.group(2)}' : '${m.group(1)}-${m.group(2)}-$tail';
      });
    }
...
class AuthTextfields {
  ...
  final effectiveFocusColor = focusColor ?? (context != null
    ? _lookupFocusColor(context) ?? Theme.of(context).colorScheme.secondary
